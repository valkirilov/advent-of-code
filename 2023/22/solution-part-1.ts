...
const findBricksSupportedByCount = (
  bricks: Array<Brick>,
  bricksSupports: Record<number, number[]>,
  bricksSupportedBy: Record<number, number[]>,
): number => {
  let bricksSupportedByCount = 0;

  for (let i = 0; i < bricks.length; i++) {
    // Count how many supports the brick has
    if (bricksSupports[i].every((j) => bricksSupportedBy[j].length >= 2)) {
      bricksSupportedByCount++;
    }
  }

  return bricksSupportedByCount;
};
...
const bricksSupportedByCount = findBricksSupportedByCount(
  movedBricks,
  bricksSupports,
  bricksSupportedBy,
);
